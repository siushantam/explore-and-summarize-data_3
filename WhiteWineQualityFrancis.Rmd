...
```{r, warning=F, fig.width=12, fig.height=30}
...
As stated above, alcohol and density are negatively correlated (r = -0.78). The above plot also seems to suggest that alcohol is correlated to quality too.  More on the later section.
...
```{r, warning=F}
ww$density.bucket <- cut(ww$density, 
  c(0.985,0.9875,0.99,0.9925,0.995,0.9975,1,1.0025,1.005))
...
It seems that the better quality white wines are more concentrated on the high alcohol but low density quadrant.
...
```{r, fig.width=8, fig.height=6, Plot_Three}
ggplot(aes(alcohol, density, color=quality), data=ww) +
...
The lower right quardrant seems to be darker.  Or using even sharper color distinction between good and bad wines:
